    ...
    mainPanel(
      tabsetPanel(type="tab",
                  tabPanel("Background/Help",
                           ...
                           p("The tab 'Classification Tree' will show you the fitted model based on the chosen parameters.The tab 'Confusion Matrix and Accuracy' will show you the results when predicting the species for the test data set using the fitted model (confusion matrix respectively various metrics)."),
                           p("Author: Alexander Schniertshauer"),
                           p("Course: Coursera - Developing Data Products")
                           
                  ),
                  tabPanel("Classification Tree",plotOutput("CTree")),
                  tabPanel("Confusion Matrix and Accuracy",verbatimTextOutput("Fit1"),verbatimTextOutput("Fit2"))
      )            
    )
  )
)
